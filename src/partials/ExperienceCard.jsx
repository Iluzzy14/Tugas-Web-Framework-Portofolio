...
const ExperienceCard = ({ isDarkMode, SortExperiences }) => {
    ...
    const currentBadgeClass = isDarkMode ? "bg-blue-900/20 text-blue-400 border-blue-900/50" : "bg-blue-50 text-blue-600 border-blue-200"
        ...
        <>
            {SortExperiences?.map((Experience) => (
                <div key={Experience.id} className="relative flex gap-6 items-start group">
                    ...
                    <div className={`w-14 h-14 ${getIconColor(Experience.is_current)} rounded-full flex items-center justify-center shadow-lg transition-transform group-hover:scale-105`}>
                        <Briefcase size={22}/>
                    </div>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 mb-3">
                        ...
                        {/* Badge */}
                        <span className={`self-start text-[10px] uppercase font-bold px-3 py-1 rounded-full border tracking-wide ${getBadgeStyle(Experience.experience_type)}`}>
